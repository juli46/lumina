...
function(){
    ...
    // ======================================
    // RECORDAR PESTAÑA ACTIVA (parámetro ?tab=)
    // ======================================

    const params = new URLSearchParams(window.location.search);
    const tabActiva = params.get("tab");

    if (tabActiva) {

        const boton = document.querySelector(
            '.tab-btn[data-tab="' + tabActiva + '"]'
        );

        const panel = document.getElementById(tabActiva);

        if (boton && panel) {
            mostrarCatalogo(tabActiva, boton);
        }
